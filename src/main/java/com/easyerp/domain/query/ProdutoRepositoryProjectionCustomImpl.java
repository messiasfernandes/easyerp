...
public class ProdutoRepositoryProjectionCustomImpl extends ServiceFuncoes implements ProdutoRepositoryProjectionCustom {
	...
	public Page<ProdutoResponse> buscarProdutos(String parametro, Pageable page) {
		...
		TypedQuery<ProdutoVariacao> variacaoQuery = entityManager.createQuery(
				"SELECT DISTINCT  v FROM ProdutoVariacao v LEFT JOIN FETCH v.atributos a   WHERE v.produto.id IN :produtoIds",
		...
		Map<Long, Set<ProdutoVariacaoResponse>> variacoesPorProduto = variacoes.stream()
				...
				.collect(Collectors.groupingBy(ProdutoVariacaoResponse::id, Collectors.toSet()));

		TypedQuery<Produto> produtoQuery = entityManager.createQuery(
				"SELECT p FROM Produto p LEFT JOIN FETCH p.marca  LEFT JOIN FETCH p.subCategoria c  LEFT JOIN FETCH p.variacoes v   WHERE p.id IN :produtoIds",
				Produto.class).setParameter("produtoIds", produtoIds);
		...
		List<ProdutoResponse> resultados = produtosList.stream().map(p -> new ProdutoResponse(p.getId(),
				p.getProdutoNome(),
